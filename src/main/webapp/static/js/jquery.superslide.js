...
$(function () {
    jQuery(".J_starCarousel").slide({
        prevCell: ".more .control-prev",
        nextCell: ".more .control-next",
...
var Usrmain = {
    bannerSwitch: function () {
        ...
        var timerPoint = null;
        var timerAuto = null;
        var $bannerTextOffset = 15;
        var $bannerContNum = $("#banner .banner-list li.banner-img").length;

        function winChange() {
            $winWidth = $(window).width();
            $("#banner .point-btn-wrap").css("left", ($winWidth - 80) / 2);
            $("#banner .banner-list li.banner-img").width($winWidth).css("left", $winWidth);
            $("#banner .banner-list").width($winWidth * 3).css("margin-left", -$winWidth);
            if ($winWidth <= 768) {
                $bannerTextOffset = 0;
                var $positionH = $winWidth * 9 / 16;
                $("#banner .banner-list li.banner-img").css({
                    "background": function (index, value) {
                        return ("url(" + $(this).attr("data-mob-src") + ")")
                    }, "padding-top": 0, "height": $positionH
                });
                $("#banner").css("height", $positionH);
                ...
                $("#banner .banner-list li.banner-img").css({
                    ...
                    }, "padding-top": 0, "height": 500
                });
                $("#banner").css("height", 500);
                $("#banner.inner-pages").css("height", 500);
                $("#banner.inner-pages li.banner-img").css("height", 500);
                $("#banner .point-btn-wrap").css({"top": "auto", "bottom": 20})
            }
        }

        $(window).bind("resize", winChange);
        winChange();
        if ($(".inner-pages .banner-list li").css("background-image") == "none") {
            return
        } else {
            $("#banner.inner-pages .banner-list .container, #banner.inner-pages .point-btn-wrap").fadeIn(500)
        }
        function bannerChangeRight() {
            $bannerCount++;
            if ($bannerCount >= $bannerContNum) {
                $bannerCount = 0
            }
            $("#banner .banner-list li.banner-img").eq($bannerCount).css({
                "z-index": 5,
                "left": 2 * $winWidth,
                "display": "block"
            }).siblings().css("z-index", 0);
            $("#banner .banner-list").stop(true, false).animate({"margin-left": -2 * $winWidth}, 800, function () {
                $(this).css({"margin-left": -$winWidth});
                $("#banner .banner-list li.banner-img").eq($bannerCount).css("left", $winWidth).find(".banner-text").animate({
                    "left": $bannerTextOffset,
                    "opacity": 1
                }, 500).find(".text").delay(100).animate({"left": 0}, 500)
            });
            $("#banner .point-btn-wrap a").eq($bannerCount).addClass("active").siblings().removeClass("active")
        }

        function bannerChangeLeft() {
            $bannerCount--;
            if ($bannerCount < 0) {
                $bannerCount = $bannerContNum - 1
            }
            $("#banner .banner-list li.banner-img").eq($bannerCount).show().css({
                "z-index": 5,
                "left": 0
            }).siblings().css("z-index", 0);
            $("#banner .banner-list").stop(true, false).animate({"margin-left": 0}, 800, function () {
                $(this).css({"margin-left": -$winWidth});
                $("#banner .banner-list li.banner-img").eq($bannerCount).css("left", $winWidth).siblings().hide()
            });
            $("#banner .point-btn-wrap a").eq($bannerCount).addClass("active").siblings().removeClass("active")
        }

        function bannerPointchange($thisIndex, $previousIndex) {
            if ($thisIndex > $previousIndex) {
                $bannerCount = $thisIndex - 1;
                bannerChangeRight()
            } else {
                $bannerCount = $thisIndex + 1;
                bannerChangeLeft()
        ...
        if ($bannerContNum > 1) {
            ...
            $("#banner").addClass("MoreBanner")
        } else {
            $("#banner .point-btn-wrap").remove();
            $("#banner a.left,#banner a.right").remove()
        }
        $("#banner.MoreBanner").mouseover(function () {
            if (!timerAuto) {
                return
            }
            clearInterval(timerAuto);
            timerAuto = null;
            $(this).find("a.switchingBtn").show()
        }).mouseout(function () {
            if (timerAuto) {
                return
            }
            ...
            $(this).find("a.switchingBtn").hide()
        });
        $("#banner a.right").click(function () {
            clearTimeout(timer);
            if ($("#gLanguageCurrent").val() == "ar-sa") {
                timer = setTimeout(bannerChangeLeft, 300)
            } else {
                timer = setTimeout(bannerChangeRight, 300)
            }
        });
        ...
        $("#banner .point-btn-wrap a").click(function () {
            ...
            clearTimeout(timer);
            var $previousIndex = $("#banner .point-btn-wrap a.active").index();
            var $thisIndex = $("#banner .point-btn-wrap a").index(this);
            $(this).addClass("active").siblings().removeClass("active");
            timerPoint = setTimeout(function () {
                bannerPointchange($thisIndex, $previousIndex)
            }, 300)
        });
        ...
        if (!$.browser.msie) {
            var target = $("#banner");
            var target1 = $("#banner li.banner-img")
        }
    }
};
